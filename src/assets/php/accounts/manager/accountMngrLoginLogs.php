...
$conn->selectQuery('*','tbl_acc_manager_login_logs');
...
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	...
	$conn->selectQuery('acl.*, CONCAT(ac.firstname," ", ac.lastname) AS fullname','tbl_acc_manager_login_logs acl LEFT JOIN tbl_client_accounts ac ON acl.id_name = ac.id '.$search.' ');
	$conn->fields = null;
...
$conn->selectQuery("acl.*, CONCAT(ac.firstname,' ', ac.lastname) AS fullname","tbl_acc_manager_login_logs acl LEFT JOIN tbl_client_accounts ac ON acl.id_name = ac.id ".$search." ORDER BY date_log DESC LIMIT ".$requestData['start']." ,".$requestData['length']." ");
